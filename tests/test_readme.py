...
class TestReadme:
    ...
    def test_range_value_example(self):
        query = RequestQueryBuilder().orbit_number(range_value("1", "2")).build()
    ...
    def test_query_sentinel_products_response_examples(self):
        ...
        failed_response.on_success(lambda data: print(f"success:{data}")).on_failure(
            lambda failure_response: print(f"failure:{failure_response.status_code}")
        )  # failure:400
    ...
    def test_request_query_builder_build_example(self):
        ...
        hanging_operator_start = (
            RequestQueryBuilder().and_().platform_name(PlatformName.SENTINEL_1).build()
        )

        assert hanging_operator_start == "platformname:Sentinel-1"

        hanging_operator_end = (
            RequestQueryBuilder()
            .not_()
    ...
    def test_request_query_example(self):
        simple_q = RequestQueryBuilder().platform_name(PlatformName.SENTINEL_1).build()
        ...
        multiple_clauses_q = (
            ...
            .and_()
            .footprint(
                "POLYGON((-4.53 29.85, 26.75 29.85, 26.75 46.80,-4.53 46.80,"
                "-4.53 29.85))"
            )
            .build()
        )
